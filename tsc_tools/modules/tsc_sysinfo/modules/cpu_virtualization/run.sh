...
if [[ -z "$(lsmod | awk '/^kvm\s/{print $4}')" ]]; then
    ...
    mkdir -p "${warn_dir}"
    LOGWARNING "${module_cname}": "$(lsmod | grep -P "^kvm")" &>/dev/null
    log_file="${warn_dir}"/"${module_name}".log LOGWARNING "${module_cname}": "$(lsmod | grep -P "^kvm")"
    LOGDELIVERY "${module_cname}" "未关闭" "$(lsmod | grep -P "^kvm")" \
        >"${result_dir}"/"${module_name}"_delivery.log
    exit 1
fi
