...
CheckArcconf() {
        ...
        #判断LD是否健康
        ld=($(cat /tmp/arcconf_array_info.txt | grep "LogicalDevicenumber"))
        ld_num=${#ld[@]}
        ...
        for ((i = 0; i < $pd_num; i++)); do
                ...
                for ((j = 0; j < $pd_warns_num; j++)); do
                        ...
                        if [[ "$pd_health" =~ "$pd_warn_status" ]]; then
                                realsolt=$(echo ${pd[i]} | awk -F ";" '{print $1}' | awk -F "#" '{print $2}')
        ...
        if [ $whole_health = 0 ]; then
                echo "信息:  Raid卡状态正常" >/tmp/arcconf_array_info.txt 2>/dev/null
        fi
...
CheckHp() {
        rm -rf /tmp/hp_array_info.txt 2>/dev/null
        array=($(hpssacli controller all show | awk -F "Slot" '{print $2}' | awk '{print $1}' | sed '/^$/d'))
        array_num=${#array[@]}
        for ((i = 0; i < $array_num; i++)); do
                hpssacli ctrl slot=${array[i]} logicaldrive all show detail | grep -E "Logical Drive:|Status" | grep -vE "Initialization|OS|MultiDomain" | sed 'N;s/\n/;/g' | sed 's/ //g' | sed 's/^/Raid卡'${array[i]}'&;/g' >>/tmp/hp_array_info.txt 2>/dev/null
                hpssacli ctrl slot=${array[i]} pd all show detail | grep -E "physicaldrive|Status" | grep -vE "Native Block Size|Authentication" | sed 's/ //g' | sed 'N;s/\n/;/g' | sed 's/^/Raid卡'${array[i]}'&;/g' >>/tmp/hp_array_info.txt 2>/dev/null
        done

        #判断LD是否健康
        ld=($(cat /tmp/hp_array_info.txt 2>/dev/null | grep "LogicalDrive"))
        ld_num=${#ld[@]}
        for ((i = 0; i < $ld_num; i++)); do
                ld_health=$(echo ${ld[i]} | awk -F ":" '{print $NF}')
                num=$(echo ${ld[i]} | awk -F ":" '{print $2}' | awk -F ";" '{print $1}')
                for ((j = 0; j < $ld_warns_num; j++)); do
                        ld_warn_status=$(echo ${ld_warns[j]} | awk -F "|" '{print $1}')
                        ld_warn_level=$(echo ${ld_warns[j]} | awk -F "|" '{print $2}')
                        shopt -s nocasematch
                        if [[ "$ld_health" =~ "$ld_warn_status" ]]; then
                                sed -i "/LogicalDrive:$num;/s/^/$ld_warn_level:  /" /tmp/hp_array_info.txt 2>/dev/null
                        fi
                done
        done
        sed -i 's/LogicalDrive/虚拟磁盘/g;s/Status/虚拟磁盘状态/g' /tmp/hp_array_info.txt 2>/dev/null
        #判断PD是否健康

        pd=($(cat /tmp/hp_array_info.txt | grep "physicaldrive"))
        pd_num=${#pd[@]}
        for ((i = 0; i < $pd_num; i++)); do
                pd_health=$(echo ${pd[i]} | awk -F ":" '{print $NF}')
                num=$(echo ${pd[i]} | awk -F ":" '{print $3}' | awk -F ";" '{print $1}')
                for ((j = 0; j < $pd_warns_num; j++)); do
                        pd_warn_status=$(echo ${pd_warns[j]} | awk -F "|" '{print $1}')
                        pd_warn_level=$(echo ${pd_warns[j]} | awk -F "|" '{print $2}')
                        shopt -s nocasematch
                        if [[ "$pd_health" =~ "$pd_warn_status" ]]; then
                                realslot=$(echo ${pd[i]} | awk -F ";" '{print $2}' | awk -F ":" '{print $3}')
                                sed -i "/physicaldrive....:$realslot;/s/^/$pd_warn_level:  /" /tmp/hp_array_info.txt 2>/dev/null
                        fi
                done
        done
        sed -i "/^[a-zA-Z]/s/^/未知状态:  /" /tmp/hp_array_info.txt 2>/dev/null
        whole_health=$(cat /tmp/hp_array_info.txt 2>/dev/null | grep -cE "告警|严重|未知状态")
        if [ $whole_health = 0 ]; then
                echo "信息:  Raid卡状态正常" >/tmp/hp_array_info.txt 2>/dev/null
        fi
        sed -i 's/physicaldrive/槽位/g;s/Status/磁盘状态/g' /tmp/hp_array_info.txt 2>/dev/null
}

CheckLsi() {
        : >/tmp/lsi_array_info.txt 2>/dev/null
        Nc=$(/opt/MegaRAID/storcli/storcli64 show | grep "Number of Controllers" | awk '{print $NF}')

        for ((i = 0; i < $Nc; i++)); do
                cn=$i
                /opt/MegaRAID/storcli/storcli64 /c${cn} show >/tmp/lsi_array_info.tmp 2>/dev/null
                #频道VD是否健康
                vdnum=$(cat /tmp/lsi_array_info.tmp | grep "Virtual Drives" | awk '{print $NF}')
                gnum=$(echo $vdnum+5 | bc)
                cat /tmp/lsi_array_info.tmp | grep -A $gnum "VD LIST" | tail -n $vdnum | while read line; do
                ...
                cat /tmp/lsi_array_info.tmp | grep -A $dnum "PD LIST" | tail -n $pdnum | while read line; do
...
stat=$(lspci | grep -iE "raid|lsi|adaptec" | wc -l)
if [ $stat -lt 1 ]; then
...
if [ $stat1 -ge 1 ]; then
        ...
        CheckLsi
elif [ $stat3 -ge 1 ]; then
        CheckHp
fi
cat /tmp/arcconf_array_info.txt 2>/dev/null
cat /tmp/hp_array_info.txt 2>/dev/null
